...
def test_draw_legend():
    ...
    assert legend_lines == list(_draw_legend(legend))


def test_draw_title():
    title_str = next(_draw_title(title="My", x_display_max=39, left_pad=0))
...
def test_draw_x_axis(axis, expected_axis_lines):
    x_lines = list(_draw_x_axis(x_axis=axis, left_pad=0))
